...
const fullText1 = splitLines("Gửi Giao,...", 3);
const fullText2 = splitLines("tui biết em còn nhiều áp lực", 3);
const fullText3 = splitLines("cảm ơn em đã đến bên anh ", 3);
const fullText4 = splitLines("và đã sưởi ấm trái tim này", 3);
const fullText5 = splitLines("khi nào giao giải quyết xong stress", 3);
const fullText6 = splitLines("mình tiếp tục mqh này nhé", 3);
const fullText7 = splitLines("tui chưa muốn dừng đâu", 3);
const fullText8 = splitLines("việc của giao là tập trung làm nó", 3);
const fullText9 = splitLines("đừng lo anh đợi em ", 3);
const fullText10 = splitLines("cuối cùng là.......", 3);
const fullText11 = splitLines("chúc giao valentine thật vui và hạnh phúc", 3);
const fullText12 = splitLines("From:Khiêm--To:giao ", 3);
...
function generateAllTargetDots() {
  ...
  const startY =
    sectionTop + (window.innerHeight - lines.length * lineHeight) / 2;
...
function shootDot() {
  ...
  if (
    targetDots.length === 0 &&
    currentCharIndex < targetDotsQueue.length - 1
  ) {
    currentCharIndex++;
  }
}
...
function animate() {
  ...
  if (
    ...
    dots.every(
      (dot) =>
        Math.abs(dot.targetX - dot.x) < 2 && Math.abs(dot.targetY - dot.y) < 2,
    )
  ) {
    animationDone = true;
    ...
    setTimeout(() => {
      ...
      if (currentTextIndex < allTexts.length) {
        ...
        if (
          bear.src !==
          "https://i.pinimg.com/originals/cf/e2/66/cfe2664925719a18a078c8c1b7552b9d.gif"
        ) {
          bear.src =
